...
{
	glColor4f(1, 1, 1, 1);
	DrawStrokeText("SHOOOOT!", SCENEMGR_INST->GetClientSize().x / 3.f, SCENEMGR_INST->GetClientSize().y - 100, 2, 0.5);
	DrawStrokeText("by psk", SCENEMGR_INST->GetClientSize().x * (1 / 3.f), SCENEMGR_INST->GetClientSize().y - 110, 2, 0.1);
	mButtons.Draw();
}
...
void SceneMainMenu::Mouse(int button, int state, int x, int y)
{
	...
	case 9:
		...
		break;
	case 10:
		SCENEMGR_INST->SceneChangeByName("MMO");
